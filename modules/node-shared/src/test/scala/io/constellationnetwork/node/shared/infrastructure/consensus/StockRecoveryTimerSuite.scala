...
/** Runs the real stock manager, storage, updater, unlock and ack timers under a virtual clock.
  * Artifact creation/validation, transport and phase readiness are test doubles.
  * This verifies timer/ack control flow, not signed artifacts or Mainnet network latency.
  */
object StockRecoveryTimerSuite extends SimpleIOSuite {
  type State = ConsensusState[Int, Int, Int, Int]
  type Storage = ConsensusStorage[IO, String, Int, String, Unit, Int, Int, Int]
  type Updater = ConsensusStateUpdater[IO, Int, String, Unit, Int, Int, Int]
  ...
  private implicit val keyLens: Lens[Int, Int] = Lens[Int, Int](identity)(_ => value => value)
  private implicit val contextLens: Lens[Int, Unit] = Lens[Int, Unit](_ => ())(_ => identity)
  ...
  private val ops = new ConsensusOps[Int, Int] {
    ...
    def kindGetter(kind: Int): PeerDeclarations => Option[PeerDeclaration] = _.proposal
  }

  private val advancer = new ConsensusStateAdvancer[IO, Int, String, Unit, Int, Int, Int] {
    ...
    def advanceStatus(resources: Resources): StateT[IO, State, IO[Unit]] = StateT { state =>
      ...
      val next = if (state.lockStatus != LockStatus.Closed && state.status < 3 && state.removedFacilitators.value.size > state.status)
    ...
    TestControl.executeEmbed {
      Supervisor[IO].use { implicit supervisor =>
        Random.scalaUtilRandomSeedInt[IO](0).flatMap { implicit random =>
          for {
            storage <- ConsensusStorage.make[IO, String, Int, String, Unit, Int, Int, Int](config)
            ...
            realUpdater = ConsensusStateUpdater.make[IO, String, Int, String, Unit, Int, Int, Int](advancer, storage, gossip, ops)
            instrumented = new Updater {
              ...
              def tryLockConsensus(key: Int, reference: State): IO[StateUpdateResult] =
                realUpdater.tryLockConsensus(key, reference).flatTap(_.traverse_ { case (_, state) =>
                  IO.monotonic.flatMap(time => lockTimes.update(_ :+ (state.status, time)))
                })
            }
            creator = new ConsensusStateCreator[IO, Int, String, Unit, Int, Int, Int] {
              def tryFacilitateConsensus(key: Int, outcome: Int, trigger: Option[ConsensusTrigger], resources: ConsensusResources[String, Int]): IO[StateCreateResult] =
                storage.condModifyState(key)(toCreateStateFn(IO.monotonic.map { time =>
                  (ConsensusState(key, outcome, Facilitators(peers), 0, time, spreadAckKinds = Set.empty[Int]), IO.unit)
                })).flatMap(evalEffect)
            }
            remover = new ConsensusStateRemover[IO, Int, String, String, Unit, Int, Int, Int](storage, gossip) {
            ...
            manager <- ConsensusManager.make[IO, String, Int, String, Unit, Int, Int, Int](
              config, storage, creator, instrumented, advancer, remover, ops, nodeStorage, clusterStorage, client
            )
            ...
            checks <- (0 until 3).toList.foldLeftM(success) { (checks, phase) =>
              ...
              } yield checks && expect.same(before.lockStatus, LockStatus.Open) &&
          ...
          } yield checks && expect.same(locks.map(_._1), List(0, 1, 2)) &&
